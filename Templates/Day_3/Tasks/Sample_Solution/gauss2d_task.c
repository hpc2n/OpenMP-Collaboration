...
#pragma omp single
	...
	{
#pragma omp task shared(integral) firstprivate(iblk) private(xpos, ypos)	  
	  {
	    double taskIntegral = 0.0;
		...
		for (int j = i ; j < NSIZE ; j++ )
		  {
		    ...
		    taskIntegral += exp(-xvecSquare) * STEPSIZE * STEPSIZE;
		  }
	      }
#pragma omp atomic
	    integral += taskIntegral;
