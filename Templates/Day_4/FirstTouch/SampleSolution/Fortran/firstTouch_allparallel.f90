Program firstTouch_lab
  ...
  !$omp parallel do default(none) shared(a, b, c) private(idbl) schedule(static)
  do i= 1, vleng
     idbl = dble(i)
     a(i) = 2.0D0 * idbl
     b(1) = 3.0D0 * idbl
  ...
  !$omp parallel do default(none) shared(c) private(idbl, compare) reduction( .and. : pass) schedule(static)
  do i=1, vleng
     idbl = dble(i)
